...
function TipBox({ label, title, body }: { label: string; title: string; body: string }) {
  return (
    <aside className="rounded-2xl border border-amber-200/80 bg-amber-50 px-4 py-3.5 sm:px-5">
  ...
  return (
    ...
    >
      ...
      {isVoice ? (
        ...
        <p className="mt-3 text-base font-medium leading-7 text-neutral-800">{section.lead}</p>
      ) : null}
      ...
      {section.networks ? (
        <ul className="mt-6 grid gap-3 sm:grid-cols-2">
          {section.networks.map((network) => (
            <li
              key={network.name}
              className="rounded-2xl border border-neutral-100 bg-[#FAFAFA] p-4"
            >
        ...
        <div className="mt-5">
          ...
          <ul className="mt-2 space-y-2">
            {section.examples.map((example) => (
              <li
                key={example}
                className="rounded-2xl border border-neutral-100 bg-white px-4 py-3 text-sm leading-6 text-neutral-800"
              >
                {guide.quoteStart}
                {example}
                {guide.quoteEnd}
...
export function GuideArticle({
  ...
  const padded = variant === "marketing" ? "px-4 py-12 sm:px-6 sm:py-16 lg:py-20" : "px-6 py-8";

  return (
    <div className={`mx-auto w-full max-w-6xl ${padded}`}>
      <header className={variant === "marketing" ? "mx-auto max-w-2xl text-center" : "max-w-2xl"}>
        ...
        <p className="mt-3 text-base leading-7 text-neutral-500 sm:text-lg">{guide.subtitle}</p>
        <p className="mt-4">
          <a
            href={guide.pdfHref}
            download
      ...
      <div className="mt-10 grid gap-10 lg:mt-14 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-14">
        <nav
          aria-label={guide.toc}
          className={variant === "marketing" ? "lg:sticky lg:top-20 lg:self-start" : "lg:sticky lg:top-4 lg:self-start"}
        >
          ...
          <ol className="flex gap-2 overflow-x-auto pb-2 lg:block lg:space-y-1 lg:overflow-visible lg:pb-0">
            {guide.sections.map((section, index) => (
              <li key={section.id} className="shrink-0">
                <a
                  href={`#${section.id}`}
                  className="flex items-start gap-2 rounded-full px-3 py-1.5 text-sm text-neutral-600 hover:bg-[#FFF4F0] hover:text-[#FF4713] lg:rounded-xl"
                >
                  <span className="mt-0.5 w-5 shrink-0 whitespace-nowrap text-[11px] tabular-nums text-[#FF4713]">
